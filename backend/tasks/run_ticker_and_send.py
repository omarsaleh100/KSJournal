#!/usr/bin/env python3
"""
KSJournal Ticker + Newsletter
Fetches live market data (after market open) then sends the newsletter.
Runs after market open so ticker has fresh prices.
Usage: python tasks/run_ticker_and_send.py
"""
import subprocess
import sys
import os
...
def run_all():
    print(f"{'='*60}")
    print(f"  THE KEELE STREET JOURNAL - Ticker & Newsletter")
    ...
    # Summary
    print(f"\n{'='*60}")
    print("  TICKER & NEWSLETTER REPORT")
    print(f"{'='*60}")
